...
class Tree:
  ...
  def fold_up(self, f, leaf):
    l = self.l.fold_up(f, leaf) if self.has_left() else leaf
    r = self.r.fold_up(f, leaf) if self.has_right() else leaf
    return f(self.v, l, r)

  def fold_up_tree(self, f, leaf):
  ...
  def fold_down_tree(self, f, root):
    root_l, root_r = f(self.v, root)
    l = self.l.fold_down_tree(f, root_l) if self.has_left() else None
    r = self.r.fold_down_tree(f, root_r) if self.has_right() else None
  ...
  def get_pos_embedding2(self, mu_l, mu_r, lam, pad_len):
    dtype = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor
    lam  =  lam.type(dtype)
    ...
    f_in = lambda _, l, r: (mu_l @ l) * (mu_r @ r) * embed_dim
    f_out = \
      lambda p, in_l, in_r: \
        (torch.einsum("i,ij,i->j", p, mu_l, mu_r @ in_r) * embed_dim, \
         torch.einsum("i,i,ij->j", p, mu_l @ in_l, mu_r) * embed_dim)

    f_in_aux = lambda v, l, r: (v, l[0], r[0])
    f_out_aux = lambda in_lr, p: (f_out(p[0], in_lr[1], in_lr[2]), in_lr[0])
    f_mult = lambda oi: oi[0] * oi[1]

    pe = self
    pe = pe.fold_up_tree(f_in, lam)
    pe = pe.fold_up_tree(f_in_aux, (lam, None, None))
    pe = pe.fold_down_tree(f_out_aux, (lam, None))
    pe = pe.map(f_mult)
    pe = pe.flatten()
  ...
  def get_pos_embedding(self, mu_l, mu_r, lam, pad_len):
    ...
    mu_r = mu_r.type(dtype)
    embed_dim = lam.size()[-1]
    f = lambda _, p: (regularize(mu_l @ p), regularize(mu_r @ p))
    pe = self.fold_down_tree(f, lam).flatten()
    pe += [torch.zeros(embed_dim).type(dtype)] * (pad_len - len(pe))
    return torch.stack(pe).type(dtype) * ((embed_dim / 2) ** 0.5)
